...
# --- Normalização de campos de custo ---

def infer_min_max_from_node(node):
    mins = []
    maxs = []
    if isinstance(node, dict):
        if 'tipos' in node and isinstance(node['tipos'], list):
            for t in node['tipos']:
                if isinstance(t, dict) and 'custo' in t:
                    c = t['custo']
                    if isinstance(c, (int, float)):
                        mins.append(c); maxs.append(c)
                        ...
                        if m:
                            ...
                            n = re.search(r"(\d+)", c)
                            if n:
                                mins.append(int(n.group(1))); maxs.append(int(n.group(1)))
        if 'custos_por_sentido' in node and isinstance(node['custos_por_sentido'], dict):
            for v in node['custos_por_sentido'].values():
...
def normalize_cost_field(node, key='custo_base'):
    if key not in node:
        return
    val = node[key]
    ...
    if isinstance(val, str):
        s = val.strip()
        # plain number with optional notes
        m_num_paren = re.match(r'^(\d+)(?:\s*\((.*)\))?$', s)
        if m_num_paren:
            num = int(m_num_paren.group(1))
            notes = m_num_paren.group(2)
            node[key] = num
            if notes:
                node['custo_base_notes'] = notes.strip()
            return
        # range like 1-5
        ...
        if m_range:
            ...
            node[key] = None
            node['custo_base_variavel'] = True
            node['custo_base_min'] = a
            node['custo_base_max'] = b
            return
        ...
        if '/' in s:
            ...
            if nums:
                ...
                node[key] = None
                node['custo_base_variavel'] = True
                node['custo_base_min'] = min(nums)
                node['custo_base_max'] = max(nums)
                node['custo_base_options'] = nums
                return
        # contains word Variável (or variavel)
        if re.search(r'vari[áa]vel', s, re.I):
            a,b = infer_min_max_from_node(node)
            node[key] = None
            node['custo_base_variavel'] = True
            if a is not None:
                node['custo_base_min'] = a
                node['custo_base_max'] = b
            return
        # leading number with notes
        m_leading = re.match(r'^(\d+)(.*)$', s)
        if m_leading:
            num = int(m_leading.group(1))
            notes = m_leading.group(2).strip()
            node[key] = num
            if notes:
                node['custo_base_notes'] = notes
            return


def recurse(o):
    if isinstance(o, dict):
        # normalize tipos' custo
        if 'tipos' in o and isinstance(o['tipos'], list):
            for t in o['tipos']:
                if isinstance(t, dict) and 'custo' in t and isinstance(t['custo'], str):
                    s = t['custo'].strip()
                    m_range = re.search(r'(\d+)\s*-\s*(\d+)', s)
                    if m_range:
                        t['custo_min'] = int(m_range.group(1))
                        t['custo_max'] = int(m_range.group(2))
                        t['custo'] = None
                    else:
                        m_num = re.search(r'(\d+)', s)
                        if m_num:
                            t['custo'] = int(m_num.group(1))
        # normalize custos_por_sentido
        if 'custos_por_sentido' in o and isinstance(o['custos_por_sentido'], dict):
            for k,v in list(o['custos_por_sentido'].items()):
                if isinstance(v, str):
                    m_range = re.search(r'(\d+)\s*-\s*(\d+)', v)
                    if m_range:
                        o['custos_por_sentido'][k + '_min'] = int(m_range.group(1))
                        o['custos_por_sentido'][k + '_max'] = int(m_range.group(2))
                        o['custos_por_sentido'][k] = None
                    else:
                        m_num = re.search(r'(\d+)', v)
                        if m_num:
                            o['custos_por_sentido'][k] = int(m_num.group(1))
        # normalize custo_base
        if 'custo_base' in o:
            normalize_cost_field(o, 'custo_base')
        # normalize generic 'custo'
        if 'custo' in o and isinstance(o['custo'], str):
            s = o['custo'].strip()
            m_range = re.search(r'(\d+)\s*-\s*(\d+)', s)
            if m_range:
                a=int(m_range.group(1)); b=int(m_range.group(2))
                o['custo_min'] = a; o['custo_max'] = b; o['custo'] = None
            else:
                m_num = re.search(r'(\d+)', s)
                if m_num:
                    o['custo'] = int(m_num.group(1))
        # recurse
...
# Run normalization on the data structure
recurse(data)
